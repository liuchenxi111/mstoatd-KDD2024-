...
class train_mst_oatd:
    ...
    def validation(self):
        ...
        with torch.no_grad():
            for batch in self.outliers_loader:
                ...
                likelihood_t = - self.detec(output_t.reshape(-1, output_t.shape[-1]),
                                            times.to(self.device).reshape(-1))
        ...
        all_likelihood_s = torch.cat(all_likelihood_s, dim=0)
        all_likelihood_t = torch.cat(all_likelihood_t, dim=0)
        pr_auc = auc_score(self.labels, (1 - all_likelihood_s * all_likelihood_t).cpu().detach().numpy())
        pr_auc_s = auc_score(self.labels, (1 - all_likelihood_s).cpu().detach().numpy())
        print("PR_AUC_S:", pr_auc_s)
