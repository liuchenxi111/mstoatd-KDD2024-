...
def get_z(trajs):
    ...
    loader = DataLoader(dataset=data, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn, num_workers=4)
    MST_OATD_U.train_loader = loader
    return MST_OATD_U.get_hidden().cpu()


def load_gmm():
    gmm_name = 'models/gmm_update_{}'.format(args.dataset)
    gmm = GaussianMixture(n_components=args.n_cluster, covariance_type='diag')
    gmm.precisions_cholesky_ = np.load(gmm_name + '_precisions_cholesky.npy')
    gmm.weights_ = np.load(gmm_name + '_weights.npy')
    gmm.means_ = np.load(gmm_name + '_means.npy')
    gmm.covariances_ = np.load(gmm_name + '_covariances.npy')
    return gmm
...
def main():
    random.seed(1234)
    train_trajs = np.load('./data/{}/train_data_init.npy'.format(args.dataset), allow_pickle=True)[-args.train_num:]
    ...
    if args.dataset == 'porto':
        for i in range(1, 11):
            train_trajs_new = np.load('./data/{}/train_data_{}.npy'.format(args.dataset, i), allow_pickle=True)
            test_trajs = np.load(
                './data/{}/outliers_data_{}_{}_{}_{}.npy'.format(args.dataset, i, args.distance, args.fraction, args.obeserved_ratio),
                allow_pickle=True)
            outliers_idx = np.load(
                "./data/{}/outliers_idx_{}_{}_{}_{}.npy".format(args.dataset, i, args.distance, args.fraction, args.obeserved_ratio),
                allow_pickle=True)
            ...
            cats_sample = random.sample(cats, args.n_cluster // 3)
    ...
    print('------------------------')
    # np.save("./results_{}/prauc_{}_{}_{}_{}_{}.npy".format(args.dataset, args.dataset, args.update_mode, args.distance,
    #                                                        args.fraction, args.obeserved_ratio), np.array(all_pr_auc))
...
if __name__ == "__main__":
    ...
    MST_OATD = train_mst_oatd(s_token_size, t_token_size, None, None, None, args.embedding_size,
                              args.hidden_size, args.device, args.n_cluster, args.dataset, args.s1_size,
                              args.s2_size, args.pretrain_lr_s, args.pretrain_lr_t, args.lr_s, args.lr_t)
    ...
    MST_OATD_U = train_mst_oatd(s_token_size, t_token_size, None, None, None, args.embedding_size,
                                args.hidden_size, args.device, args.n_cluster, args.dataset, args.s1_size,
                                args.s2_size, args.pretrain_lr_s, args.pretrain_lr_t, args.lr_s, args.lr_t)
