...
class MST_OATD(nn.Module):
    ...
    def forward(self, trajs, times, lengths, batch_size, mode, c):
        ...
        if mode == "pretrain" or "train":
            ...
            self.W2.data /= sum_W
            self.W3.data /= sum_W
            # encoder_final_state = torch.stack([encoder_final_state_s1, encoder_final_state_s2,
            #                                    encoder_final_state_s3],dim=2).squeeze(0)
            # encoder_final_state = self.state_att(encoder_final_state).unsqueeze(0)
